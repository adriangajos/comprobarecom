...
export function buildProformaPdf({ proformaNumber, order, totals, dates, paid = false }) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor(GREY).fontSize(10).font("Body")
      ...
      .text(paid ? `Payment made on: ${dates.issue}` : `Pay by: ${dates.due}`, { width: right - left, align: "right" });
    ...
    if (paid) {
      ...
      doc.text("Status: PAID (waiting for confirmation)");
    ...
    doc.fillColor(DARK).fontSize(9.5).font("Body")
      .text(
        paid
          ? "This is a proforma invoice. We are waiting for payment confirmation. The " +
